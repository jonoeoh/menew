...
class ValidationsTest < ActiveModel::TestCase
  ...
  def test_invalid_options_to_validate
    ...
    message = "Unknown key: :presence. Valid keys are: :on, :if, :unless, :prepend, :except. Perhaps you meant to call `validates` instead of `validate`?"
  ...
  def test_validate_with_except
    Topic.validates :title, presence: true, except: :custom_context
  ...
  def test_validations_some_with_except
    Topic.validates :title, presence: { except: :custom_context }, length: { maximum: 10 }
